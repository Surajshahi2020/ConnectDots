...
def keyboard_onlinekhabar_to_json(request):
    """Main function for Online Khabar scraper with WebSocket support"""
    base_url = "https://www.ujyaaloonline.com/"
    ...
    # Settings
    DELAY = 2  # Reduced delay for faster scraping
    MAX_WORKERS = 4
    ...
    def get_user_keywords(request):
        ...
        try:
            ...
            # Fallback Nepali keywords for Online Khabar
            fallback = {
                ...
                'सुरक्षा': 'Security', 'हत्या': 'Crime', 'चोरी': 'Crime', 'दुर्घटना': 'Accident'
            }
    ...
    def find_article_links(session):
        """Find article links from Online Khabar"""
        nonlocal current_step
        update_progress("Finding articles", 0, 1, "🔍 Searching for articles in Online Khabar sections...")
        # --------------------------------------------------------------------------Change fetching category-------------------------------------------------------------
        categories = [
            "", "content/news/rastiya", "content/news/", "opinion", "business", "trend", "/category/opinion", "category/politics", "/category/social", "/desh-paradesh"
        ]
        ...
        for idx, category in enumerate(categories):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Online Khabar specific article selectors
                    article_selectors = [
                        ".post__text", ".post__title", "article", ".news__box",
                        ".item-news", ".main__news", ".featured-news", ".regular-news",
                        "div.news", "div.post", "div.article-item"
                    ]
                    ...
                    for selector in article_selectors:
                        ...
                        for element in elements:
                            ...
                            # Filter URLs
                            if ('onlinekhabar.com' not in href or
                                any(x in href.lower() for x in ['category', 'tag', 'author', 'page', '?', 'search', 'feed']) or
                                href in seen_urls or '#' in href or href.endswith('/')):
                                continue
                            
                            # Get title
                            title = ""
                            title_elem = element.find(['h2', 'h3', 'h4', 'h1'])
                            if title_elem:
                            ...
                            if len(title) >= 10 and href.startswith('http') and 'content' not in href.lower():
                                articles.append({'url': href, 'title': title[:250]})
                                seen_urls.add(href)
                                
                                if len(articles) >= 70:  # Limit for Online Khabar
        ...
        # Also check homepage for more articles
        try:
            update_progress("Finding articles", len(categories), len(categories), 
                           "🏠 Checking homepage for articles...")
            response = session.get(base_url, timeout=15)
            if response.status_code == 200:
                soup = BeautifulSoup(response.content, 'html.parser')
                
                # Find trending/news sections
                sections = soup.select('.trending-news, .latest-news, .featured-news, .main-news')
                for section in sections:
                    for link in section.find_all('a', href=True):
                        href = link.get('href', '').strip()
                        if href and href.startswith('/') and 'onlinekhabar.com' not in href:
                            href = base_url + href
                        
                        if ('onlinekhabar.com' in href and 
                            'content' in href and 
                            href not in seen_urls and
                            'author' not in href and 'category' not in href):
    ...
    def extract_date_from_onlinekhabar(soup, url):
        """Extract date from Online Khabar article"""
        try:
            ...
            date_selectors = [
                ...
                ".post__time", ".published-date", ".article-date",
                "time", ".date", "span.date"
            ]
                ...
                if elem:
                    ...
                    if date_str:
                        ...
                        date_patterns = [
                            ...
                            r'(\d{4}/\d{2}/\d{2})',
                            r'(\d{2}-\d{2}-\d{4})',
                            r'(\d{1,2}\s+[माघ|फागुन|चैत|बैशाख|जेठ|असार|साउन|भदौ|आश्विन|कार्तिक|मंसिर|पुष]\s+\d{4})',
                        ]
                        ...
                        for pattern in date_patterns:
                            ...
                            if match:
                                try:
                                    date_str_clean = match.group(1)
                                    # Convert Nepali month names if needed
                                    nepali_months = {
                                        'बैशाख': '01', 'जेठ': '02', 'असार': '03', 'साउन': '04',
                                        'भदौ': '05', 'आश्विन': '06', 'कार्तिक': '07', 'मंसिर': '08',
                                        'पुष': '09', 'माघ': '10', 'फागुन': '11', 'चैत': '12'
                                    }
                                    
                                    for nep_month, num_month in nepali_months.items():
                                        if nep_month in date_str_clean:
                                            # Extract year and day
                                            year_match = re.search(r'(\d{4})', date_str_clean)
                                            day_match = re.search(r'(\d{1,2})', date_str_clean)
                                            if year_match and day_match:
                                                year = year_match.group(1)
                                                day = day_match.group(1).zfill(2)
                                                date_str_clean = f"{year}-{num_month}-{day}"
                                            break
            ...
            print(f"Date extraction error: {str(e)[:50]}")
        
        return None
    
    def fetch_article_content(article, session, article_num, total_articles):
        """Fetch content for a single Online Khabar article"""
        try:
            time.sleep(DELAY * 0.3)  # Reduced delay for Online Khabar
            ...
            # Get date
            pub_date = extract_date_from_onlinekhabar(soup, article['url'])
            
            # Filter by date
            if pub_date and pub_date < TWO_DAYS_AGO:
                return None
            
            # Get content - Online Khabar specific content selectors
            content = ""
            
            # Try multiple content selectors for Online Khabar
            content_selectors = [
                "div.post__content", "div.news-details", "article div.content",
                "div.article-content", "div.entry-content", "div.description",
                "div.main__content", ".detail-content", "div.text"
            ]
            ...
            for selector in content_selectors:
                ...
                if elem:
                    ...
                    for unwanted in elem.select('.advertisement, .related-news, .comments-section, script, style, iframe, .social-share, .news-related'):
                        unwanted.decompose()
                    ...
                    if paragraphs:
                        text_parts = []
                        for p in paragraphs[:25]:  # Limit paragraphs
                            text = p.get_text(strip=True)
                            # Filter out short text, ads, and navigation
                            if (len(text) > 30 and 
                                not text.startswith(('ADVERTISEMENT', 'SPONSORED', 'सम्बन्धित', 'पढ्नुहोस्', 'यो पनि')) and
                                '©' not in text and 'प्रकाशित' not in text[:20]):
                                text_parts.append(text)
                        
                        if text_parts:
                            content = ' '.join(text_parts)[:3500]  # Increased content limit for Nepali
                            break
            
            # Fallback: try to get all meaningful paragraphs
            if not content or len(content) < 100:
                ...
                if all_paragraphs:
                    ...
                    for p in all_paragraphs[:20]:
                        text = p.get_text(strip=True)
                        if len(text) > 40 and not any(x in text for x in ['ADVERTISEMENT', 'SPONSORED', '©']):
                            text_parts.append(text)
                    if text_parts:
                ...
                if elem:
                    ...
                    if src and 'http' in src:
                        ...
                        if '150x150' in image_url:
                            image_url = image_url.replace('150x150', '800x600')
                        break
            ...
            category = "General"
            if 'onlinekhabar.com' in article['url']:
                # Try to extract category from URL
                url_parts = article['url'].split('/')
                if len(url_parts) >= 4:
                    possible_category = url_parts[3]
                    if possible_category in ['/content/news', 'news', 'politics', 'economy', 'sports', 'entertainment',
                                            'national', 'international', 'opinion', 'health', 'education',
                                            'technology', 'auto', 'lifestyle']:
                        category = possible_category.capitalize()
                else:
                    # Try to get category from page
                    cat_elem = soup.select_one('.breadcrumb a, .category a, .post__cat')
                    if cat_elem:
            ...
            return {
                ...
                'language': 'nepali'  # Online Khabar is primarily Nepali
            }
    ...
    def analyze_keywords_in_article(article, keyword_dict):
        """Find which keywords match in the article (supports Nepali)"""
    ...
    def filter_by_keywords(articles, keyword_dict):
        ...
        for idx, article in enumerate(articles):
            ...
            urgent_categories = ['Violence', 'Terrorism', 'Emergency', 'Crime', 'Accident']
            high_categories = ['Politics', 'Government', 'Protest', 'Economic', 'Security']
            medium_categories = ['Education', 'Health', 'Environment', 'Legal', 'Army']
            ...
            # Also check content for emergency keywords (Nepali)
            content_lower = article['content'].lower()
            emergency_keywords_nepali = ['आपतकाल', 'आपदा', 'दुर्घटना', 'हत्या', 'आक्रमण', 'बम', 'विस्फोट', 'मृत्यु', 'अपहरण']
            if any(kw in content_lower for kw in emergency_keywords_nepali):
                threat_level = "high"
                priority = "high"
                ...
                try:
                    ...
                    news_article = AutoNewsArticle(
                        ...
                        metadata=json.dumps({
                            ...
                            'language': article.get('language', 'nepali'),
    ...
    # MAIN EXECUTION
    try:
        print("Hamro Patro news fetching started")
        send_to_websocket("=" * 60)
        send_to_websocket("🚀 STARTING ONLINE KHABAR SCRAPER")
        ...
        send_to_websocket(f"🇳🇵 Language: Nepali")
        ...
        send_to_websocket("=" * 60)
        send_to_websocket("🎯 ONLINE KHABAR SCRAPING COMPLETE - FINAL SUMMARY")
...
def fetch_onlinekhabar_news(request):
    """
    Main function to call from your keyboard view
    """
    try:
        # Call the scraper function
        result_json = keyboard_onlinekhabar_to_json(request)
        result_data = json.loads(result_json)
        
        return {
            "status": result_data["metadata"]["status"],
            "message": result_data["metadata"].get("message", "Online Khabar fetch completed"),
            "stats": {
        ...
        return {
            "status": "error",
            "message": f"Error in fetch_onlinekhabar_news: {str(e)}"
        }
...
if __name__ == "__main__":
    ...
    print("🚀 Testing Online Khabar Scraper...")
    
    # Note: This won't work without proper Django setup
    # For testing, use the fetch_onlinekhabar_news function
    print("✅ Online Khabar scraper code created successfully")
    print("📝 Main function: keyboard_onlinekhabar_to_json(request)")
    print("📝 Integration function: fetch_onlinekhabar_news(request)")